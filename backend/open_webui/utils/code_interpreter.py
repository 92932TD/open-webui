...
class MarimoCodeExecuter:
    """
    Execute code in Marimo notebook
    """
    ...
    def __init__(
        ...
        """
        :param base_url: Marimo server URL (e.g., "http://localhost:2718")
        :param code: Code to execute
        :param token: Marimo authentication token (optional)
        :param timeout: WebSocket timeout in seconds (default: 60s)
        """
        self.base_url = base_url
        self.code = code
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if self.kernel_id:
            try:
                async with self.session.delete(
                    f"api/kernel/{self.kernel_id}", params=self.params
                ) as response:
                    response.raise_for_status()
            except Exception as err:
                logger.exception("close kernel failed, %s", err)
    ...
    async def sign_in(self) -> None:
        # token authentication - Marimo uses Authorization header
        if self.token:
            self.session.headers.update({"Authorization": f"Bearer {self.token}"})

    async def init_kernel(self) -> None:
        async with self.session.post(url="api/kernel", params=self.params) as response:
            response.raise_for_status()
            kernel_data = await response.json()
            self.kernel_id = kernel_data["kernel_id"]

    def init_ws(self) -> (str, dict):
        ws_base = self.base_url.replace("http", "ws", 1)
        websocket_url = f"{ws_base}api/kernel/{self.kernel_id}/ws"
        ws_headers = {}
        
        # Pass Authorization header to WebSocket if token is used
        if self.token:
    ...
    async def execute_code(self) -> None:
        ...
        async with websockets.connect(
            ...
            await self.execute_in_marimo(ws)

    async def execute_in_marimo(self, ws) -> None:
        # send message
        msg_id = uuid.uuid4().hex
        await ws.send(
            json.dumps(
...
async def execute_code_marimo(
    base_url: str, code: str, token: str = "", timeout: int = 60
) -> dict:
    async with MarimoCodeExecuter(base_url, code, token, timeout) as executor:
        result = await executor.run()
        return result.model_dump()
